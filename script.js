...
════════════════════════════════ */
function updateNavigationUI(name, membership = null) {
  ...
  if (window.isLoggedIn && name) {
    ...
    if (membership) {
      ...
      badgeHtml = `<span style="background:${badgeColor};color:white;font-size:0.7rem;padding:2px 8px;border-radius:12px;font-weight:bold;text-transform:uppercase;">${membership}</span>`;
    }
    navAuth.innerHTML = `
...
async function handleLogin() {
  ...
  try {
    ...
    if (userData.password === password) {
      ...
      updateNavigationUI(userData.nama, userData.membership || null);
...
function renderMyBookings() {
  ...
  container.innerHTML = myBookings.map(b => {
    ...
    // Tentukan warna dot sensor saat render awal
    const dotClass = getSensorDotClass(b);
    ...
    <div class="schedule-card">
      <div class="schedule-header">
        ...
        <div class="schedule-header-right">
          ...
          ${dotClass ? `<span class="sensor-dot ${dotClass}" data-sensor="${b._docId || ''}"></span>` : ''}
        </div>
      </div>
...
/* ════════════════════════════════
   LIVE COURT SENSOR — DOT MERAH/HIJAU
   Merah = sekarang dalam jam sesi, Hijau = tidak
════════════════════════════════ */
const SLOT_DURATION_H = 2;
const BULAN_ID = {
...
function isSlotActiveNow(booking) {
  if (!booking.date || !booking.time) return false;
  const d = parseBookingDate(booking.date);
  if (!d) return false;
  const [h, m] = booking.time.split(':').map(Number);
  const start  = new Date(d); start.setHours(h, m, 0, 0);
  const end    = new Date(start); end.setHours(end.getHours() + SLOT_DURATION_H);
  const now    = Date.now();
  return now >= start.getTime() && now < end.getTime();
}

// Kembalikan class CSS: 'sensor-dot-red', 'sensor-dot-green', atau '' (membership/cancelled)
function getSensorDotClass(b) {
  if (b._type === 'membership' || b.status === 'Dibatalkan') return '';
  return isSlotActiveNow(b) ? 'sensor-dot-red' : 'sensor-dot-green';
}

// Update warna dot setiap 30 detik tanpa rebuild seluruh kartu
function updateLiveSensors() {
  myBookings.forEach(b => {
    if (b._type === 'membership' || b.status === 'Dibatalkan' || !b._docId) return;
    const dot = document.querySelector(`.sensor-dot[data-sensor="${b._docId}"]`);
    if (!dot) return;
    const newClass = getSensorDotClass(b);
    dot.className = `sensor-dot ${newClass}`;
  });
}

function startSensorInterval() {
  if (sensorInterval) clearInterval(sensorInterval);
  sensorInterval = setInterval(updateLiveSensors, 30000);
}

// Stub agar tidak error kalau ada kode lama yang memanggilnya
function buildSensorHTML() { return ''; }
function fmtCountdown()    { return ''; }
function getSlotStatus()   { return null; }
